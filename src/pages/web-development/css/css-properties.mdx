...
<Callout type="info">
  #### Points to note 1. By default, `align-self` aligns items in the block
  direction (downwards). If you want to align items in the inline direction
  (left to right), use `justify-self` or `justify-items` instead. 2. Block and
  inline direction depend on the language. For example, inline direction for
  Arabic would be right to left. 3. The `align-self` property overrides the
  `align-items` property. 4. The `align-self` property can take in the following
